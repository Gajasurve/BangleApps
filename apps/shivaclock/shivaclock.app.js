...
function loadDayTithi(key) {
  return streamDayArray("tithi.json", key);
}

function loadDayNak(key) {
  return streamDayArray("nakshatra.json", key);
}
...
function drawAll() {
  ...
  g.setColor("#FFA500");
  g.setFontAlign(0,-1);
  g.drawString(getTopLabel(),88,22);
...
function init() {
  ...
  // 🔴 THIS WAS MISSING — restores motion & wake
  Bangle.setOptions({
    wakeOnTwist: true,
    twistThreshold: 1500,
    twistMaxY: -1500,
